...
import { 
  ...
  Clock, Map, HeartHandshake, Image as ImageIcon, Star, Check, Copy, Edit2 
} from "lucide-react";
...
export default async function SubmissionDetailPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const formatDate = (date: Date | null) => date ? new Date(date).toLocaleDateString('en-IN', { timeZone: 'Asia/Kolkata', dateStyle: 'medium' }) : null;
  ...
  const editUrl = `${copyUrl}&editId=${submission.id}`;
      ...
      <div className="max-w-5xl mx-auto space-y-6">
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <div className="flex flex-wrap items-center gap-2">
            ...
            <Link href={editUrl} className="flex items-center gap-2 px-4 py-1.5 bg-amber-50 hover:bg-amber-100 text-amber-700 border border-amber-200 rounded-md text-xs font-black uppercase tracking-widest shadow-sm transition-colors">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="space-y-6">
            <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6">
              ...
              <div className="mb-4">
                <span className="inline-flex items-center gap-1.5 px-2.5 py-1 rounded-md text-xs font-bold bg-slate-100 text-slate-600 border border-slate-200 uppercase tracking-wider font-mono">
                  ID: {referenceNumber}
                </span>
              </div>
